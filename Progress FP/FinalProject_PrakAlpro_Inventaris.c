...
int transaksibarang(){
	...
	if(fptr == NULL){
		...
		for(int i = 0; i < currentItems; i++){
			...
			if(bandingkan == 0){ //strcmp bernilai 0 jika hasil perbandingannya sama.
				int konversiHarga;
				int konversiJumlah;
				konversiHarga = atoi(barang[i].harga); //konversi dari string ke int
				konversiJumlah = atoi(barang[i].jumlah); //konversi dari string ke int
				printf("Barang :\n");
				...
				itoa(sisa, sisaBuffer, 10); //integer desimal variabel "sisa" ini dikonversi menjadi string ascii lalu masuk ke var sisaBuffer
				strcpy(barang[i].jumlah, sisaBuffer); //string pada sisaBuffer disalin atau diassign ke atribut jumlah pada struct barang
			...
			}else{ //strcmp bernilai selain 0 (1) jika hasil perbandingannya ada yg tidak sama.
				continue; //lanjutkan looping mencari kode yang tepat
			}
